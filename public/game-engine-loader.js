(()=>{
  ...
  const fail=(detail)=>{
    ...
    if(p)p.textContent='The 3D engine could not load from any backup source. '+detail+' Try a hard refresh (Ctrl+Shift+R).';
  };
  const sources=[
    'https://cdn.jsdelivr.net/npm/three@0.181.1/build/three.module.js',
    'https://unpkg.com/three@0.181.1/build/three.module.js?module',
    'https://esm.sh/three@0.181.1'
  ];
  async function loadThree(){
    let last;
    for(let i=0;i<sources.length;i++){
      try{
        status(i===0?'Loading 3D engine…':`Trying 3D backup ${i}…`);
        const mod=await Promise.race([
          import(sources[i]),
          new Promise((_,reject)=>setTimeout(()=>reject(new Error('engine source timed out')),6500))
        ]);
