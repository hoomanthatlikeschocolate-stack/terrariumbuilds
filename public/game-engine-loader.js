(()=>{
  ...
  function isTester(u){if(!u)return false;const name=String(u.username||'').toLowerCase();return !!(u.owner===true||u.isOwner===true||String(u.role||'').toLowerCase()==='owner'||name==='aiden'||name==='greyson');}
  ...
  async function loadThree(){
    const attempts=[['/api/three.js?build=14','TerrariumBuilds engine'],['https://cdn.jsdelivr.net/npm/three@0.160.1/build/three.module.js','jsDelivr backup'],['https://unpkg.com/three@0.160.1/build/three.module.js','unpkg backup']];
    const errors=[];
  ...
  function patchGame(code){
    // Existing parser safety repairs.
    ...
    // Larger world + additional streets.
    ...
    // Fix traffic cars being rotated sideways.
    code=code.replace('c.g.rotation.y=p.ang;','c.g.rotation.y=p.ang+Math.PI/2;');

    // Bigger/taller house shell and taller town buildings.
    ...
    code=code.replace("const traffic=[{...makeCar(0x5e7385),route:0,u:.05,s:9.5},{...makeCar(0x9b594b),route:1,u:.42,s:8.2},{...makeCar(0x7a7d68),route:2,u:.69,s:10.2}];",
      "const traffic=[{...makeCar(0x5e7385),route:0,u:.05,s:9.5},{...makeCar(0x9b594b),route:1,u:.42,s:8.2},{...makeCar(0x7a7d68),route:2,u:.69,s:10.2}];const ownedCar=makeCar(0x365e78);ownedCar.g.position.set(HX+13,0,HZ+HD/2+9);ownedCar.g.rotation.y=0;ownedCar.speed=0;");
    code=code.replace("const BASE={version:72,username:'Guest'","const BASE={version:72,inCar:false,username:'Guest'");

    // Let previously collected feeder insects satisfy the later objective at the habitat.
    ...
    // Insert car detection before normal outside interactions.
    ...
    // Replace walking movement with walking + actual car controls.
    code=code.replace(/function move\(dt\)\{[\s\S]*?\}\nfunction updateDoors/,`function move(dt){
      ...
      if(state.inCar){
        ...
        ownedCar.speed+=gas*dt*13;
        ownedCar.speed*=Math.pow(.985,dt*60);
        if(keys.Space)ownedCar.speed*=Math.pow(.82,dt*60);
        ownedCar.speed=clamp(ownedCar.speed,-7,22);
        ...
        ownedCar.g.position.addScaledVector(f,ownedCar.speed*dt);
        ownedCar.g.position.x=clamp(ownedCar.g.position.x,-305,305);ownedCar.g.position.z=clamp(ownedCar.g.position.z,-305,305);
      ...
      const moving=x||z,sprint=!!((keys.ShiftLeft||keys.ShiftRight)&&state.energy>2);
...
function updateDoors`);
    ...
    // Camera follows car; indoors stays below the opaque roof instead of peeking through it.
...
let ht=`);
    ...
    // Map UI, M shortcut, objective marker and player/car marker.
    ...
    // Fresh unsaved testing mode for owners/testers. No normal save is read or overwritten.
    if(testing){
  ...
  async function runGame(THREE){
    ...
    const r=await fetch('/game-v007-r2.js?build=14',{cache:'no-store'});
    if(!r.ok)throw new Error('game file returned HTTP '+r.status);
    let code=patchGame(await r.text());
    let fn;
    try{fn=new Function('THREE',code+'\n//# sourceURL=terrariumbuilds-update-007-r3.js');}
    catch(err){throw new Error('game syntax check failed: '+(err?.message||String(err)));}
    status('Building larger town, cars and house…');
    let ready=false;window.addEventListener('tb3d-ready',()=>{ready=true;},{once:true});
  ...
  async function boot(){
    ...
    if(testing&&!isTester(sessionUser)){testing=false;const u=new URL(location.href);u.searchParams.delete('testing');history.replaceState({},'',u);}
    installTestingButton();
