...
function tutorial(b:any){const title=clean(b.title,150),animal=clean(b.animal,70);const steps=Array.isArray(b.steps)?b.steps.slice(0,30).map((s:any,i:number)=>({title:clean(s?.title,150)||'Step '+(i+1),body:clean(s?.body,5000)})).filter((s:any)=>s.body):[];if(!title||!animal||!steps.length)throw new Error('Add a tutorial title, animal, and at least one build instruction.');return{title,animal,summary:clean(b.summary,1000),difficulty:clean(b.difficulty,40)||'Beginner',materials:clean(b.materials,5000),steps,note:clean(b.note,3000),sources:Array.isArray(b.sources)?b.sources.slice(0,10).map((s:any)=>({label:clean(s.label,150),url:url(s.url)})).filter((s:any)=>s.url):[],image_url:url(b.image_url),published:b.published===true,updated_at:new Date().toISOString()};}
